-- Trailblazer CRM — v36: server-side guard so an upsell can never be created
-- on an order that's already been delivered, matching the client-side check
-- (defense in depth — this enforces it even if the request didn't go
