-- Trailblazer CRM — v29: a Set can now also be the target of a package
-- change / upsell (upgrading a customer from their original order to a
-- Set), not just something they order originally.
-- Run in Supabase: SQL Editor > New query > paste all > Run

alter table upsells add column if not exists upsell_set_id uuid references product_sets(id);
alter table upsell_commission_rules add column if not exists upsell_set_id uuid references product_sets(id);
