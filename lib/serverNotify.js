...
export async function getOrderNotifyRecipients(supabaseAdmin) {
  const { data } = await supabaseAdmin.from('profiles').select('id, role, active');
  return (data || [])
    .filter(p => p.role === 'admin' || (p.role === 'staff' && p.active !== false))
    .map(p => p.id);
}
...
const DEFAULT_CONFIRMATION_TEMPLATE =
  "Hi {customer}, we've received your order ({order_short}) and it's being processed.{track_line} — Trailblazer";
...
export async function sendOrderConfirmationMessages(supabaseAdmin, { phone, customerName, orderId, sendSms, sendWhatsapp: wantWhatsapp, customMessage }) {
  ...
  if (wantWhatsapp) results.push(await sendWhatsApp(phone, customerName, orderId, trackLink));
