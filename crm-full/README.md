...
## Upgrading an already-deployed CRM to the new features

If you've already deployed this once and are just adding the new features
(reports, inventory, remarks/history, customer history, notifications):

1. In Supabase, go to **SQL Editor > New query**, paste in the contents of
   `supabase/migration_v2.sql`, and click **Run**. This adds the new columns
   and tables without touching your existing data.
2. Re-upload the changed/new files to your GitHub repo (`app/dashboard/page.js`,
   the new `app/dashboard/features.js`, and `app/globals.css`) — same
   drag-and-drop process as before, into the same `crm-full` folder structure.
3. In Vercel, go to **Deployments** and click **Redeploy** on the latest one.
4. Refresh your CRM link — you'll see new nav items (Inventory, Reports),
   new order fields, and a "History" link on every order.
