...
import { STATUSES, logEvent, orderTotal, sendConfirmation, forwardToDispatchCompany, ReportsPage, InventoryPage, OrderHistoryModal, CustomerHistoryModal, NotificationsBell, NIGERIA_STATES, AgentStockPage, MyStockPage, ConfirmOrderModal, SettingsPage, SubmitterView } from './features';
...
export default function Dashboard() {
  ...
  async function refreshAll() {
    const [{ data: prod }, { data: ord }, { data: profs }, { data: stock }, { data: settingsRows }, { data: companies }] = await Promise.all([
  ...
  const roleLabel = { admin: 'Admin', staff: 'Staff', dispatch: 'Dispatch partner', manager: 'Manager', logistics: 'Logistics Manager', marketer: 'Marketer' }[profile.role] || profile.role;
  ...
  return (
    <div className="app">
      ...
      <div className="main">
        ...
        {isAdmin && page === 'orders' && <OrdersPage orders={orders} products={products} profiles={profiles} isAdmin profile={profile} settings={settings} dispatchCompanies={dispatchCompanies} refresh={refreshAll} />}
        ...
        {profile.role === 'staff' && page === 'dashboard' && <OrdersPage orders={myOrders} products={products} profiles={profiles} title="My orders" myId={profile.id} myRole="staff" profile={profile} settings={settings} dispatchCompanies={dispatchCompanies} refresh={refreshAll} />}
...
function OrderModal({ products, order, onSave, onClose }) {
  ...
  const giftProduct = selectedProduct ? products.find(p => p.id === selectedProduct.gift_product_id) : null;
...
function OrdersPage({ orders, products, profiles, isAdmin, title, myId, myRole, profile, settings, dispatchCompanies, refresh }) {
  ...
  async function createOrder(fields) {
    ...
    if (!error && data) {
      ...
      if (product) {
        ...
        if (product.gift_product_id && fields.gift_quantity > 0) {
          const gift = products.find(p => p.id === product.gift_product_id);
          if (gift) await supabase.from('products').update({ stock_quantity: Math.max(0, gift.stock_quantity - fields.gift_quantity) }).eq('id', gift.id);
        }
      }
            ...
            {filtered.map(o => (
              <tr key={o.id}>
                ...
                <td style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                  {isAdmin && o.status === 'New' && <><button className="link-btn" onClick={() => setConfirming(o)}>Confirm</button>{' · '}</>}
...
function ProductsPage({ products, orders, refresh }) {
  ...
  async function setGift(id, giftId) {
    await supabase.from('products').update({ gift_product_id: giftId || null }).eq('id', id);
    refresh();
  }
  return (
    <div>
      <div className="topbar"><div><h1 className="page-title">Products</h1><p className="page-sub">Each product gets its own order queue and tab. You can also link a free gift product that ships alongside it.</p></div></div>
        ...
        {products.map(p => (
          <div key={p.id} className="list-manage-row">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <select value={p.gift_product_id || ''} onChange={e => setGift(p.id, e.target.value)} style={{ fontSize: '12px', padding: '5px 8px', border: '1px solid #DEDAD0', borderRadius: '4px' }}>
                <option value="">No free gift</option>
                {products.filter(g => g.id !== p.id).map(g => <option key={g.id} value={g.id}>Gift: {g.name}</option>)}
              </select>
...
function TeamPage({ profiles, orders, products, session, refresh }) {
  ...
  async function createLogin() {
    ...
    setName(''); setEmail(''); setPassword(''); setState(''); setAllowedProducts([]);
    refresh();
  }
        ...
        {staffList.map(s => {
          ...
          return (
            <div key={s.id} className="list-manage-row">
              <span>{s.full_name} <span style={{ color: '#8A93A0', fontSize: '11.5px' }}>({s.role}{s.state ? ` · ${s.state}` : ''}) · {load} orders</span>
                {!s.active && <span className="pill Cancelled" style={{ marginLeft: '8px' }}>Inactive</span>}
              </span>
              <div style={{ display: 'flex', gap: '8px' }}>
                {submitter && <button className="link-btn" onClick={() => setEditingAccess(s)}>Edit product access</button>}
                <button className="btn" onClick={() => toggleActive(s)}>{s.active ? 'Deactivate' : 'Activate'}</button>
              </div>
            </div>
          );
        })}
